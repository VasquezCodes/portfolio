...
const experiencias = [
    ...
    },
    {
        ...
        fecha: "2025",
          icono: <Database className="w-5 h-5 text-purple-300" />,
    ...
    },
    {
        ...
        fecha: "2024",
          icono: <Code2 className="w-5 h-5 text-blue-300" />,
...
export default function Experiencia() {
    ...
    return (
        <PageTransition>
          <SectionWrapper className="text-white min-h-screen px-4 sm:px-10 pt-40 pb-32">
                <motion.h2
                    ...
                    {/* Cards de experiencia */}
                      <div className="w-full lg:w-2/3 space-y-8">
                          {experiencias.map((exp, i) => (
                              <motion.div
                                  key={i}
                                  className="relative border-l-4 rounded-r-xl border-purple-500 glass p-6 shadow-lg hover:shadow-purple-500/20 transition-all duration-300"
                                  initial={{ opacity: 0, y: 20 }}
                                  animate={{ opacity: 1, y: 0 }}
                              ...
                              >
                                  <div className="flex items-start justify-between mb-3">
                                      <div className="flex gap-3 items-center">
                                          {exp.icono}
                                          <div>
                                              <p className="text-base sm:text-lg font-bold">{exp.empresa}</p>
                                              <p className="text-sm text-indigo-200">{exp.rol} · {exp.fecha}</p>
                                          </div>
                                      </div>
                                      <button
                                          className="text-xs flex items-center gap-1 bg-white/10 hover:bg-purple-500 hover:text-white text-indigo-100 px-3 py-1 rounded transition-all"
                                  ...
                                  <p className="text-indigo-100 text-sm sm:text-base">{exp.descripcionCorta}</p>

                                  {expandido === i && (
                                      <>
                                          <ul className="mt-4 space-y-2 text-sm text-indigo-200 list-disc list-inside pl-2">
                                          ...
                                          <div className="mt-4 flex flex-wrap gap-2">
                                              {exp.tecnologias.map((tech, idx) => (
                                                  <span
                                                      key={idx}
                                                      className="text-xs border border-purple-400 text-purple-200 px-2 py-0.5 rounded-full bg-white/5"
                                                  >
                                                      {tech}
                                                  </span>
                                              ))}
                                          ...
                                          {exp.referencia && (
                                              <div className="mt-4">
                                                  <a
                                                      ...
                                                      className="inline-block px-4 py-1 text-sm border border-purple-400 text-purple-300 rounded hover:bg-purple-500 hover:text-white transition"
                                                      target="_blank"
                        ...
                        <AnimatePresence mode="wait">
                            {imagenActiva && (
                                <motion.img
                                  ...
                                  className="w-full max-w-sm rounded-xl border border-purple-500 shadow-lg shadow-purple-500/20"
                              />
                          )}
                      </AnimatePresence>
                  </div>
